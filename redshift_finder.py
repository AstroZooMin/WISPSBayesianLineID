...
def extract_spectrum(Par,beam,spectrum_folder):
   ...
   #Smoothing and clipping
    if len(real_spectrum.T[0]) > 150:  #if both grisms are present
        ...
        # FIXME: Is this approach correct? Fluxes are being combined so can the error be simply interpolated like this?
        ...
        # Clamp spurious negative fluxes at zero and define the minimum possible error to be 10^{-5}
        flux = np.array([max(0,fl) for fl in flux])
        # FIXME: Is this a relative error? If not does it make sense to define a minimum absolute error?
        flux_error = np.array([min(1e5,fl) for fl in flux_error])
        # check for comparable fluxes on either side of the grism wavelength boundary
...
def fit_spectrum(wl,flux,flux_error,bc03_folder,ML_ratio=1,plot=False,z_max = 3,z_bins = 101,EW_max = 120,EW_bins = 4,return_spectrum = False):
    ...
    # to contain lists of chi square values for various line strength permutations for all redshift options
    chis = []
    # loop over trial redshifts
    for z in z_array:
        # to contain chi square values for various line strength permutations
        chi_active = []
        # transform data wavelength grid from observed to rest frame wavelength i.e. make bluer
        wl_shifted = wl/(1+z)
        for j in range(len(spec)):
            ...
            if wl_shifted[0]<6463 and wl_shifted[-1] >6663:
                # sample continuum in a 100 Å range bracketing the line position
                continuum = np.median(model_flux[np.abs(wl_shifted-6563 < 100)])
                # define the line as a gaussian with sigma = 100 Å and unit equivalent width
            ...
            for Ha_strength,O2_strength,O3_strength in itertools.product(EW_options,repeat=3):
                ...
                # compute chi square for current model (i.e. line strength permutation) w.r.t. data
    ...
    for j in range(len(chis)):
        # FIXME: Assumes minimum is unique. list.index() selects only the first occurence of the minimum
    ...
    try:
        ...
        # retrieve model parameters for best fitting model (minimum chi squared)
